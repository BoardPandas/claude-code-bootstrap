...
- The wiring guard is `scripts/check-claude-wiring.mjs` (`npm run check:claude`; runs in CI). Intentionally-dead globs are recorded with a reason in `.claude/references/wiring-exemptions.json`; a stale exemption fails the guard.
...
- Keep this file under 16 KB (`wc -c`). Budget by bytes, not lines -- a line-count budget keeps passing while single lines grow to thousands of characters. `npm run check:claude` enforces the ceiling.
