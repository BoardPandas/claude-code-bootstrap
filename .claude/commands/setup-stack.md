...
Check if `.claude/skills/stacks/` exists. If it does:

1. **Keep** skill files relevant to the user's stack
2. **Remove** skill files for stacks the user is NOT using
3. If there are no stack-specific skill files, skip this step

### Step 4: Update skill-rules.json (if applicable)

If `.claude/skill-rules.json` exists and there are optional skills that should be enabled/disabled based on the stack, update it accordingly.
